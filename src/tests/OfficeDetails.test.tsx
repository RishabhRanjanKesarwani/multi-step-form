...
const validFieldInputs = ['Building', 'Bangalore', '1234567890', 'Address Line 1', 'Address Line 2', '0987654321'];
const invalidFieldInputs = ['Building', 'Bangalore', '12345e67890', 'Address Line 1', '', '09876e54321'];
...
describe('Office details', () => {
    it('should have 6 text inputs', () => {
        ...
        render(
            <Provider store={store}>
                <OfficeDetails onNext={mockFn} className="" />
            </Provider>
        );
        expect(screen.getAllByTestId(TEST_IDS.officeDetailsTextField).length).toEqual(6);
    });

    it('should have 5 required text inputs', () => {
        const mockFn = jest.fn();
        render(
            <Provider store={store}>
                <OfficeDetails onNext={mockFn} className="" />
        ...
        const nextButton = screen.getByTestId(TEST_IDS.officeDetailsNextButton);
        fireEvent.click(nextButton);
        let requiredFields = 0;
        screen.getAllByTestId(TEST_IDS.officeDetailsTextField).forEach(field => {
            if (field.children.length === 2) { // More than 1 means error helper text is present in the DOM for the field
                requiredFields++;
            }
        })
        expect(requiredFields).toEqual(5);
    });
    ...
    it('should show altogether 2 errors for invalid mobile number and for invalid landline number and no error for non-mandatory fields', () => {
        const mockFn = jest.fn();
        render(
            <Provider store={store}>
                <OfficeDetails onNext={mockFn} className="" />
    ...
    it('should show error message on API failure', async () => {
        ...
        const nextButton = screen.getByTestId(TEST_IDS.officeDetailsNextButton);
        fireEvent.click(nextButton);
        expect(await screen.findByTestId(TEST_IDS.officeDetailsErrorMessage)).toBeInTheDocument();
    });
});